...
impl<T: Config> Pallet<T> {
    ...
    fn get_validator_permits(netuid: u16) -> (Vec<bool>, Vec<bool>, Vec<bool>) {
        ...
        let new_validator_permits: Vec<bool> = is_topk(&stake, max_allowed_validators as usize);
        log::trace!("new_validator_permits: {:?}", new_validator_permits);
        (validator_permits, validator_forbids, new_validator_permits)
    }

    /// Calculates active stake of validators and returns this value in a given `netuid`.
    /// # Args:
    /// * 'netuid': ( u16 ):
    #[allow(clippy::indexing_slicing)]
    pub fn subtensor_active_stake(netuid: u16, exclude_uid: Option<u16>) -> Vec<I32F32> {
        ...
        if let Some(uid) = exclude_uid {
            if (uid as usize) < active_stake.len() {
                active_stake[uid as usize] = I32F32::from_num(0.0);
            }
        }
    ...
    /// Calculates reward consensus values and returns  this for uids/hotkeys in a given `netuid`.
    /// (Sparse version used for production.)
    /// # Args:
    /// * 'netuid': ( u16 ):
    pub fn subtensor_consensus(netuid: u16, exclude_uid: Option<u16>) -> Vec<I32F32> {
        let n: u16 = Self::get_subnetwork_n(netuid);
        let weights = Self::get_pre_clip_sparse_weights(netuid);
        let active_stake = Self::subtensor_active_stake(netuid, exclude_uid);
        // Compute preranks: r_j = SUM(i) w_ij * s_i
    ...
    /// Calculates bonds and bond_data and return this values for uids/hotkeys in a given `netuid`.
    /// (Sparse version used for production.)
    /// # Args:
    /// * 'netuid': ( u16 ):
    ...
    pub fn subtensor_epoch(netuid: u16, incentive: Option<bool>, exclude_uid: Option<u16>) -> EpochResult<T> {
        ...
        if incentive.unwrap_or(false) {
            // Fetching incentive storage value
            // Fetching incentive storage value
            let incentive_value: Vec<I32F32> = Self::subtensor_incentive(netuid, exclude_uid);
            return EpochResult::Incentive(incentive_value);
        }
